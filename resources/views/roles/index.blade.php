        ...
        <div class="py-8">
            <div class="mx-auto max-w-7xl sm:px-6 lg:px-8">
                ...
                <div class="overflow-hidden bg-white shadow sm:rounded-lg">
                    <div class="p-6">
                        @if(session('success'))
                            <div class="px-4 py-3 mb-4 text-sm text-green-700 bg-green-100 rounded-md">
                                {{ session('success') }}
                            </div>
                        @endif
    
                        @if(session('error'))
                            <div class="px-4 py-3 mb-4 text-sm text-red-700 bg-red-100 rounded-md">
                                {{ session('error') }}
                            </div>
                        @endif
                                    ...
                                    @forelse($roles as $role)
                                        <tr class="hover:bg-gray-50">
                                            <td class="px-6 py-4 whitespace-nowrap">
                                                <a href="/settings/roles/single-role/{{$role->id}}" class="text-indigo-600 hover:text-indigo-900 font-medium">
                                                    {{ $role->name }}
                                                </a>
                                            </td>
                                            <td class="px-6 py-4">
                                            ...
                                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                                <div class="flex justify-end space-x-4">
                                                    <a href="/settings/roles/single-role/{{$role->id}}/edit" class="text-indigo-600 hover:text-indigo-900" title="Edit">
